...
- This client has admin access.
- Admin tools can create workspaces, issue keys, revoke keys, and rotate keys.
- Workspace names must use short, clear Latin slugs.
- Do not use spaces, Cyrillic, or decorative suffixes in workspace names.
- Admin document operations may explicitly target a selected workspace for reads and writes.
- If a document belongs to a specific workspace, work inside that workspace instead of mixing projects in `main`.
- Use `main` only for shared operational notes when a dedicated workspace is not needed.
