...
Ссылка выше закреплена на последнем стабильном релизе (`@v1.1.0` — см.
[Releases](https://github.com/alexbic/lightrag-mcp-connect/releases)
для списка доступных версий). Убери `@v1.1.0` совсем, чтобы всегда
следовать за `main`, либо замени на конкретный SHA коммита, если нужна
более точная фиксация.

## Деплой (удалённый)

Это предполагает, что у тебя уже есть работающий инстанс LightRAG,
доступный из этого стека (`LIGHTRAG_URL`).
...
**Ещё нет своего LightRAG?** `docker-compose.full-example.yml` в той
же папке включает сам LightRAG рядом с этим шлюзом, в одном файле —
всё связано вместе, с плейсхолдерами из `.env` (никаких реальных
ключей в коде, как и в двух файлах выше). Это то, с чего начинать, если
разворачиваешься на чистом сервере с нуля:

```bash
cp .env.example .env
# заполни .env: DOMAIN, LIGHTRAG_API_KEY, MCP_AUTH_PASSWORD, плюс
