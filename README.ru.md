...
1. **`upload_document` работает из удалённого/изолированного агента.**
   ...
   сам скачивает) и `file_content` (base64) как альтернативы — так что
...
Ссылка выше закреплена на последнем стабильном релизе (`@v1.0.0` — см.
[Releases](https://github.com/alexbic/lightrag-mcp-connect/releases)
для списка доступных версий). Убери `@v1.0.0` совсем, чтобы всегда
