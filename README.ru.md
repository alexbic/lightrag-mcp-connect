...
Этот форк добавляет `file_content` (base64) + `filename` как
альтернативу: содержимое едет прямо внутри вызова инструмента, общая
файловая система не нужна. `file_path` оставлен для случаев, когда
MCP-сервер и клиент на одной машине (локальный stdio) — там он всё ещё
имеет смысл.
...
// После (работает независимо от того, где запущен вызывающий агент):
